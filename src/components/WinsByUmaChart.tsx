        ...
        <div style={{ display: 'flex', overflowX: 'auto', paddingBottom: 8 }}>
            ...
            <div style={{ flex: '1 0 auto' }}>
                <div style={{ position: 'relative', height: chartHeight + labelTopPadding }}>
                    ...
                    <div style={{ position: 'absolute', left: 0, right: 0, bottom: 0, display: 'flex', alignItems: 'flex-end', padding: '0 16px' }}>
                        {items.map((item) => {
                            ...
                            return <div key={item.charaId} style={{ width: 64, margin: '0 8px', display: 'flex', justifyContent: 'center' }}>
                                <div style={{ position: 'relative', height: heightPx, width: 36, borderRadius: 6, background: 'linear-gradient(180deg, #60a5fa 0%, #2563eb 100%)', boxShadow: '0 2px 10px rgba(37,99,235,0.35), inset 0 0 0 1px rgba(255,255,255,0.08)' }}>
                ...
                <div style={{ display: 'flex', padding: '8px 16px 0 16px' }}>
                    {items.map((item) => {
                        const iconUrl = getIconUrl(item.charaId) || '';
                        return <div key={item.charaId} style={{ width: 64, margin: '0 8px', display: 'flex', justifyContent: 'center' }}>
                            <div style={{ width: 36, height: 36, borderRadius: 8, background: '#0f0f0f', border: '1px solid #2a2a2a', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                                {iconUrl && <img src={iconUrl} alt="icon" style={{ width: 28, height: 28, borderRadius: 4 }} />}
                            </div>
                        </div>;
                    })}
                </div>
                ...
                {/* Names */}
                <div style={{ display: 'flex', padding: '6px 16px 0 16px' }}>
                    {items.map((item) => {
                        return <div key={item.charaId} style={{ width: 64, margin: '0 8px', textAlign: 'center' }}>
                            <div style={{ fontSize: 11, color: '#cbd5e1', overflow: 'hidden', whiteSpace: 'nowrap', textOverflow: 'ellipsis' }} title={item.name}>{item.name}</div>
                        </div>;
                    })}
                </div>

                {/* X-axis label intentionally removed */}
            </div>
        </div>
    </div>;
}
